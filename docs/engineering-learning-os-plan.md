...
このプロダクトは学習そのものを目的の一つにしているため、実装は基本的に自分の手で行う。
...
一方で、アプリケーション本体の実装コードは AI に生成させず、自分で書く。Next.js、TypeScript、Go、Rust、DB 設計、テスト、コードエディタ組み込み、将来の runner 設計を実装経験として積むことを重視する。
